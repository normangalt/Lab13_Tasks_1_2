...
class LinkedBST(AbstractCollection):
    ...
    def demo_bst(self, path):
        ...
        def search(subroot, key):
            ...
            if key < subroot.data and subroot.left is not None:
                return search(subroot.left, key)

            if key > subroot.data and subroot.right is not None:
                return search(subroot.right, key)
        ...
        with open(path, 'r', encoding='utf-8') as file:
            ...
            dictionary_list = [element[:-1] for element in dictionary_list]

        print('Searching binary tree check starts!')
        print()
        print('Words used for check have been written to the file \'looked_for.txt\'')
        print()
        print()
        print('First case: search for 10000 words in sorted list of words.')
        print('Python list searching function.')
        sorted_dictionary_list = list(sorted(dictionary_list))
        look_for = choices(sorted_dictionary_list, k = 10000)
        with open('looked_for.txt', 'w', encoding = 'utf-8') as file:
            for word in look_for:
                file.write(word)
        time_number = 0
        for word in look_for:
            start = time()
            sorted_dictionary_list.index(word)
            time_dif = time() - start
            time_number += time_dif
        print(f'Time used: {time_number} seconds')
        print()
        print('Second case: search for 10000 words in sorted list of words in a binary tree.')
        sorted_dictionary_list = list(sorted(dictionary_list))
        look_for = choices(sorted_dictionary_list, k = 10000)
        tree = LinkedBST()
        for word in look_for:
        ...
        for word in look_for:
            start = time()
            search(tree.root, word)
            time_dif = time() - start
            time_number += time_dif

        print(f'Time used: {time_number} seconds')
        print()
        print('Third case: search for 10000 words in unsorted list of words in a binary tree.')
        sorted_dictionary_list = list(sorted(dictionary_list))
        look_for = choices(sorted_dictionary_list, k = 10000)
        tree = LinkedBST()
        for word in look_for:
        ...
        print(f'Time used: {time_number} seconds')
        print()
        print('Fourth case: search for 10000 words in unsorted list\
 of words in a binary balanced tree.')
        sorted_dictionary_list = list(sorted(dictionary_list))
        look_for = choices(sorted_dictionary_list, k = 10000)
        tree = LinkedBST()
        for word in look_for:
